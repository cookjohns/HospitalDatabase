...
def roomUtilization():
  ...
  while True:
    ...
    if (num == '1'):
      c.execute('SELECT ra.roomNumber, p.firstName, p.lastName, a.date'
...
def patientInformation():
  ...
  while True:
    ...
    print('\t9.  All Patients Readmitted Within 30 Days of Their '  #NYI
          + 'Last Discharge Date')
    print('\t10. Patient Statistics') #NYI
	...
	+ 'FROM Patient;')
      ...
      c.execute('SELECT patientId, firstName, lastName'
	+ 'FROM ('
	...
	+ 'WHERE date IS NULL;')
      ...
      c.execute('SELECT p.patientId, p.firstName, p.lastName'
	+'FROM Patient p '
	+	'JOIN InPatient ip '
	...
	+ 'AND a.date < ?;', startDate, endDate)
      ...
      c.execute('SELECT a.date, d.diagnosis'
	+ 'FROM Diagnoses d'
	+	'JOIN Admits a'
	...
	+ 'WHERE p.patientId = ?;', requestedPatient)
      ...
      requestedPatient = raw_input('Enter a patient\'s name: ')
      c.execute('SELECT admit.date, t.treatmentId'
	+ 'FROM InPatient ip'
...
def diagnosisAndTreatmentInformation():
  ...
  while True:
    ...
    print('\t4.  Treatments Performed at the Hospital') #NYI
    print('\t5.  Treatments Performed on Admitted Patients')
    print('\t6.  Treatments Performed on Outpatients') #NYI
    print('\t7.  Diagnoses Associated with Patients with Highest Occurrences of ' #NYI
          + 'Addmissions')
    print('\t8.  Employees Involved in a Given Treatment Occurance') ##NYI
    
    num = raw_input('Enter a command: ')

    if (num == '1'):
      c.execute('SELECT d.diagnosisId, d.name, COUNT(DISTINCT d.diagnosesId)'
	+ 'FROM InPatient ip '
	+	'JOIN Patient p'
	...
	+'ORDER BY a.date DESC;')
      ...
      c.execute('SELECT t.treatmentId, t.name, COUNT(DISTINCT t.treatmentId)'
	+'FROM '
...
def employeeInformation():
  ...
  while True:
    ...
    print('\t3.  Primary Doctors of Patients with High Admission Rates') #NYI
    print('\t4.  All Diagnoses Given by a Specified Doctor') 
    print('\t5.  All Treatments Ordered by a Given Doctor') ##NYI
    ...
    if (num == '1'):
      c.execute('SELECT employeeId, firstName, lastName, category, hireDate'
	+'FROM ('
	...
	+'WHERE vi.dayOfWeek = \'Tuesday\';')
      ...
      c.execute('SELECT d.diagnosis, COUNT(d.diagnosis) totalOccurences'
	...
	+'ORDER BY d.date DESC;', requestedDoctor)
      ...
      requestedDoctor = raw_input('Enter a doctor\'s name: ')
      c.execute('SELECT t.treatmentId, COUNT(t.treatmentId) as totalAdministered'
	+'FROM Doctor doc'
	...
	+'ORDER BY a.timeAdministered DESC;', requestedDoctor)
      ...
      c.execute('SELECT tg.employeeId'
