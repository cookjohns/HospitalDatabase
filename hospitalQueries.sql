-- A. Room Utilization
	...
	SELECT roomNumber, firstName, lastName, timeAdmitted
...
--B. Patient Information
	...
	-- NOTE: startDate and endDate are parameters provided by user ('yyyy-mm-dd')
	-- Still doing GROUP BY because of strange admits dates
	-- WORKS!
	SELECT OutPatientNames.patientId AS patientId, 
	...
	--GROUP BY OutPatientNames.patientId;
	...
	-- NOTE: requestedPatient is a parameter provided by user
	-- Not correct at all
	SELECT t.treatmentId, admit.timeAdmitted
	FROM InPatient ip
		JOIN Administers admin
			ON ip.patientId = admin.patientId
		JOIN Treatment t
			ON admin.treatmentId = t.treatmentId
		JOIN Admits admit
			ON admit.patientId = ip.patientId
	WHERE ip.patientId = '34242'
	GROUP BY admit.timeAdmitted
	ORDER BY admit.timeAdmitted DESC, admin.timeAdministered ASC;
	
	SELECT t.treatmentId, t.name
	FROM Treatment t 
		JOIN (
			SELECT p.patientId, a.treatmentId
			FROM Patient p JOIN Administers a ON p.patientId = a.patientId) treatedPatients
		ON t.treatmentId = treatedPatients.treatmentId
	WHERE treatedPatients.patientId = '85489';
	--9)
...
--D. Employee Information
	...
	--2)
	-- I think a ton of the room tables are missing
	SELECT v.employeeId
	FROM Volunteer v
